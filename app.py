...
if menu == "승부 예측 게임":
    ...
    # 확인 버튼 누르면 경기 결과 출력
    if st.button("확인") and bet_amount > 0:
    ...
    if st.session_state.show_result:
        ...
        if winner == st.session_state.selected_team:
            # 배당률에 따라 머니 계산
            if winner == home_team:
                win_money = st.session_state.bet_amount * home_odds
            else:
                win_money = st.session_state.bet_amount * away_odds
