...
if menu == "승부 예측 게임":
    ...
    if idx >= len(matches):
        ...
        st.session_state.bet_processed = False
        st.session_state.money_updated = False
        st.experimental_rerun()  # 라운드 넘어가므로 새로고침
    ...
    if st.button("다음 경기"):
        st.session_state.winners.append(st.session_state.winner)
